...
static esp_err_t zb_attribute_handler(const esp_zb_zcl_set_attr_value_message_t *message)
{
    ...
    if (message->info.dst_endpoint == HA_ESP_VALVE_ENDPOINT) {
        if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_ON_OFF) {
            if (message->attribute.id == ESP_ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID && message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_BOOL) {
                light_state = message->attribute.data.value ? *(bool *)message->attribute.data.value : light_state;
                ESP_LOGI(TAG, "Valve command: %s", light_state ? "OPEN" : "CLOSE");
                valve_set_state(light_state ? VALVE_OPEN : VALVE_CLOSED);
            }
        }
    }
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_set_rx_on_when_idle(true);   //evita que el ESP se duerma antes de finalizar la entrevista
    esp_zb_sleep_set_threshold(500);    // puede aumentarse si es necesario
